...
<template>
  <div class="container-wrapper" :class="[customClass]" v-show="!widget.options.hidden">
    <slot></slot>

    <div class="container-action" v-if="designer && designer.selectedId === widget.id && !widget.internal">
      <i :title="i18nt('designer.hint.selectParentWidget')" @click.stop="selectParentWidget(widget)">
        <svg-icon icon-class="el-back" />
      </i>
      <i v-if="!!parentList && parentList.length > 1" :title="i18nt('designer.hint.moveUpWidget')" @click.stop="moveUpWidget()">
        <svg-icon icon-class="el-move-up" />
      </i>
      <i v-if="!!parentList && parentList.length > 1" :title="i18nt('designer.hint.moveDownWidget')" @click.stop="moveDownWidget()">
      ...
      <i v-if="widget.type === 'table'" :title="i18nt('designer.hint.insertColumn')" @click.stop="appendTableCol(widget)">
        <svg-icon icon-class="el-insert-column" />
      </i>
      <i v-if="widget.type === 'grid' || widget.type === 'table'" :title="i18nt('designer.hint.cloneWidget')" @click.stop="cloneContainer(widget)">
    ...
    <div class="drag-handler" v-if="designer && designer.selectedId === widget.id && !widget.internal">
