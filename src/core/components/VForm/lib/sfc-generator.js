import { isNotNull } from "@/core/utils/tool";
import { genVue2JS } from "@/core/components/VForm/lib/vue2js-generator";
import { beautifierOpts } from "@/core/components/VForm/lib/beautifierLoader";
import { genVue3JS } from "@/core/components/VForm/lib/vue3js-generator";
    ...
    .map((tr) => {
      return `<tr>${tr.cols
        ...
        .map((td) => {
          ...
          let tdStyleAttr =
            tdStyleArray.length > 0 ? `style="${tdStyleArray.join(";")}"` : "";
...
const genScopedCSS = function(formConfig, vue3Flag = false) {
  ...
  .label-left-align ${
    vue3Flag ? `:deep(.el-form-item__label)` : `:deep(.el-form-item__label)`
  } {
    text-align: left;
  }
...
export const genSFC = function(
  ...
  const html = beautifier.html(
    genTemplate(formConfig, widgetList, vue3Flag),
    beautifierOpts.html
  );
  const js = beautifier.js(
    vue3Flag
      ? genVue3JS(formConfig, widgetList)
      : genVue2JS(formConfig, widgetList),
    beautifierOpts.js
  );
  const globalCss = beautifier.css(
    genGlobalCSS(formConfig),
    beautifierOpts.css
  );
  const scopedCss = beautifier.css(
    genScopedCSS(formConfig, vue3Flag),
    beautifierOpts.css
  );
