<template>
  <div class="content-table-wrap mt-20">
    ...
    <div class="table-wrap">
      <div style="display:flex; margin-bottom: 3px; ">
        <div style="border: 1px solid #ebeef5;margin-right: 5px;" id="parseSql">
          <textarea ref="$textarea"></textarea>
        </div>
        <el-button type="primary" :icon="Search" @click="handleParserDataModel"
          >一键解析</el-button
        >
      </div>
      <div style="margin-bottom: 10px;">
        ...
        <el-button
          type="danger"
          :icon="Delete"
          :disabled="selectTableName === '' && selectedRowIds === ''"
          @click="handleDelModelCol"
        >
          删除
        </el-button>
      </div>
      <el-table
        ...
        @selection-change="handleSelectChangeData"
      >
        ...
        <el-table-column align="center" label="操作">
          <template #default="scope">
            <el-button type="primary" link @click="handleEditAttr(scope.row)"
              >编辑</el-button
            >
          </template>
        </el-table-column>
  ...
  <DataModelEdit
    ref="dataModelEditRef"
    @refreshModelList="refreshModelList"
    :moduleData="moduleData"
  ></DataModelEdit>
  <DataModelColEdit
    ref="dataModelPropertyEditRef"
    @refreshModelColList="refreshModelColList"
    :rowData="rowData"
...
const options = ref({
  ...
  placeholder:
    "请输入create table... 或 alter table...[add|drop|rename|change]... DDL语句",
});
...
const tranlateStatementFn = async () => {
  try {
    ...
    const {code, data, msg} = await tranlateStatement(params)
    if (code === '200') {
      $message.notifySuccess(data+'解析成功')
      selectTableName.value = data
      getModelListFn()
...
const handleDeleteModel = () => {
  ...
  let ids = idsArr.join(";");
  if (ids) {
    $message.confirmAction("确定要删除该数据模型？", "提示", () => {
      deleteModelFn(ids);
    });
  } else {
...
const handleDelModelCol = async () => {
  $message.confirmAction("确定要删除选中的项目？", "提示", () => {
    deleteModelColFn();
  });
};
...
const updateModel = async () => {
  $message.confirmAction("确定要同步更新选中的数据模型？", "提示", () => {
    updateModelFn();
  });
};
