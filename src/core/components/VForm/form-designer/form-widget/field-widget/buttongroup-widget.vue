<template>
  ...
  >
    <div ref="fieldEditor" :class="{ clearfix: field.options.displayStyle == 'block' }" :style="{ display: field.options.displayStyle }">
      <vxe-button-group :style="{ float: field.options.pageAlign == 'left' ? 'left' : 'right' }">
        <vxe-button
      ...
      >
        <vxe-option v-for="item in selectOptions" :key="item.value" :label="item.label" :value="item.value" />
      </vxe-select>
    </div>
  </static-content-wrapper>
</template>
...
export default {
  ...
  computed: {
    buttonItems() {
      return this.field.options.items.slice(0, this.field.options.moreIndex || 5)
    },
    selectOptions() {
      return this.field.options.items.length > (this.field.options.moreIndex || 5) ? this.field.options.items.slice(this.field.options.moreIndex || 5) : []
    }
  },
  beforeCreate() {
    /* 这里不能访问方法和属性！！ */
  ...
  methods: {
    handleClick(items, e) {
      if (this.field.options.onButtonGroupClick) {
        let customFn = new Function('ButtonConfig', this.field.options.onButtonGroupClick)
