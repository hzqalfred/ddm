...
export function comUtils(CT) {
    ...
    function deleteGridData(signName) {
        ...
        const moduleCode = BI.FMC;
        const functionCode = BI.FFC;
        const { field, $refs: { [signName]: $table } } = BI.AC[signName];
        if ($table.getInsertRecords().length) return BIM.Message.notifyError("有未保存的新增数据！");
        if ($table.getUpdateRecords().length) return BIM.Message.notifyError("有未保存的修改数据！");
        ...
        const performDelete = async (data) => {
            ...
            return await BIM.execRequest(`${functionCode}.delete`, { moduleName, moduleCode, data: dataArray });
        };
    ...
    function saveGridData(signName) {
        ...
        const handleRequest = async (type, records) => {
            try {
                ...
                await BIM.execRequest(`${functionCode}.save`, { moduleName, moduleCode, data: records });
                BI.CC.comUtils.loadGridData(signName);
                BIM.Message.notifySuccess(`${type === 'insert' ? '新增' : '修改'}成功！`);
            } catch (error) {
    ...
    function saveFormData(signName, isRC, gridName) {
        ...
        var row = drawer.selRow;
        var data = BI.CC.comUtils.getFormData(row, BI.AC);
        BIM.execRequest(`${functionCode}.save`, { moduleName, moduleCode, data: [data] })
            .then((res) => {
                if(gridName){
