<template>
  <el-form-item prop="name" :rules="nameRequiredRule">
    ...
    <template v-if="!!selectedWidget.category || noFieldList">
      <el-input type="text" :disabled="disabled" v-model="optionModel.name" @change="updateWidgetNameAndRef"></el-input>
    </template>
    <template v-else>
      <el-select :disabled="disabled" v-model="optionModel.name" allow-create filterable @change="updateWidgetNameAndRef" :title="i18nt('designer.setting.editNameHelp')">
